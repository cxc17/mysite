...
class ByrbbsPipeline(object):
    ...
    def user_insert(self, tx, item):
        sql = 'INSERT INTO `user` (`user_id`, `user_name`, `gender`, `astro`, `qq`, `msn`, `home_page`, `level`, ' \
              '`post_count`, `score`, `life`, `last_login_time`, `last_login_ip`, `status`, `face_url`, ' \
              '`face_height`, `face_width`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'

        tx.execute(sql, (item['user_id'], item['user_name'], item['gender'], item['astro'], item['qq'], item['msn'],
                         item['home_page'], item['level'], item['post_count'], item['score'], item['life'],
                         item['last_login_time'], item['last_login_ip'], item['status'], item['face_url'],
                         item['face_height'], item['face_width']))
